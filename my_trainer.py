...
def run_train_bpe(
    ...
    **kwargs,
    ...
    while heap and merges_done < target_merges:
        ...
        # Tie-break: prefer pairs without newlines; then lexicographically largest by bytes
        def tie_key(p: tuple[int,int]):
            a_b = id_to_bytes[p[0]]
            b_b = id_to_bytes[p[1]]
            has_nl = (b"\n" in a_b) or (b"\n" in b_b)
            # We want non-newline pairs first => use (has_nl) in key with max(), so False > True
            return (not has_nl, a_b, b_b)
        best_pair = max(same_freq_valid, key=tie_key)
