...
def cross_entropy_loss(inputs: Float[Tensor, " batch_size sequence_length vocab_size"], targets: Int[Tensor, " batch_size sequence_length"]) -> Float[Tensor, ""]:
    ...
    log_normalizer: Float[Tensor, " batch_size sequence_length"] = torch.log(torch.sum(torch.exp(inputs), dim=-1))
    # Select the logit corresponding to the target class for each example / batch
    losses: Float[Tensor, " batch_size sequence_length"] = log_normalizer - inputs.gather(dim=-1, index=targets.unsqueeze(-1)).squeeze(-1)
